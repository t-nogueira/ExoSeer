...
function App() {
  ...
  const generatePassportPDF = (candidate, analysis) => {
    try {
      const candidateData = {
        name: candidate.name,
        radius_earth: candidate.radius_earth,
        orbital_period: candidate.orbital_period,
        transit_depth: candidate.transit_depth,
        star_temperature: candidate.star_temperature,
        discovery_method: candidate.discovery_method,
        discovery_year: candidate.discovery_year,
        status: candidate.status,
        host_star: candidate.host_star
      };

      // Create comprehensive passport content
      const passportContent = `
ExoSeer Candidate Passport
========================

Candidate: ${candidate.name}
Status: ${candidate.status || 'Candidate'}
Discovery Year: ${candidate.discovery_year || 'Unknown'}
Discovery Method: ${candidate.discovery_method || 'Transit'}

Planetary Parameters:
- Radius: ${candidate.radius_earth?.toFixed(2)} Earth radii
- Orbital Period: ${candidate.orbital_period?.toFixed(3)} days
- Transit Depth: ${(candidate.transit_depth * 100)?.toFixed(4)}%
- Semi-major Axis: ${candidate.semi_major_axis?.toFixed(3)} AU

Stellar Parameters:
- Host Star: ${candidate.host_star || 'Unknown'}
- Stellar Temperature: ${candidate.star_temperature || 'Unknown'} K
- Stellar Radius: ${candidate.star_radius?.toFixed(2)} Solar radii
- Stellar Mass: ${candidate.star_mass?.toFixed(2)} Solar masses

Analysis Summary:
${analysis ? `
- Light Curve SNR: ${analysis.light_curve_analysis?.snr?.toFixed(1)}
- Transit Duration: ${analysis.physics_analysis?.duration_hours?.toFixed(2)} hours
- Impact Parameter: ${analysis.physics_analysis?.impact_parameter?.toFixed(3)}
- Validation Score: ${analysis.validation?.validation_score?.toFixed(3)}
- False Positive Probability: ${(analysis.validation?.false_positive_probability * 100)?.toFixed(2)}%
...
Generated by: ExoSeer v1.2.3
      ...
      a.href = url;
      a.download = `${candidate.name?.replace(/\s/g, '_') || 'exoplanet'}_passport.txt`;
  ...
  const exportAnalysisData = (candidate, analysis) => {
    try {
      const csvData = [
        ['Parameter', 'Value', 'Unit', 'Description'],
        ['Name', candidate.name, '', 'Candidate designation'],
        ['Radius', candidate.radius_earth?.toFixed(3), 'Earth radii', 'Planet radius'],
        ['Period', candidate.orbital_period?.toFixed(6), 'days', 'Orbital period'],
        ['Transit Depth', (candidate.transit_depth * 100)?.toFixed(6), '%', 'Transit depth'],
        ['Host Star Temperature', candidate.star_temperature, 'K', 'Stellar effective temperature'],
        ['Discovery Year', candidate.discovery_year, '', 'Year of discovery'],
        ['Status', candidate.status, '', 'Validation status']
      ];

      if (analysis) {
        csvData.push(
          ['SNR', analysis.light_curve_analysis?.snr?.toFixed(3), '', 'Signal-to-noise ratio'],
          ['Duration', analysis.physics_analysis?.duration_hours?.toFixed(3), 'hours', 'Transit duration'],
          ['Impact Parameter', analysis.physics_analysis?.impact_parameter?.toFixed(3), '', 'Impact parameter'],
          ['Validation Score', analysis.validation?.validation_score?.toFixed(3), '', 'Overall validation score']
        );
      }
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${candidate.name?.replace(/\s/g, '_') || 'exoplanet'}_analysis.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  const shareAnalysis = (candidate, analysis) => {
    try {
      const shareText = `Check out this exoplanet: ${candidate.name} - A ${candidate.radius_earth?.toFixed(1)}x Earth-sized planet orbiting every ${candidate.orbital_period?.toFixed(0)} days! Analyzed with ExoSeer: Advanced AI Exoplanet Detection & Vetting v1.2.3`;
      
      if (navigator.share) {
        navigator.share({
          title: `ExoSeer Analysis: ${candidate.name}`,
          text: shareText,
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-3 text-sm">
              <span className={`text-xs font-medium ${userMode === 'novice' ? 'text-white exoseer-glow-text' : 'text-white/60'}`}>Novice Mode</span>
              <Switch 
                checked={userMode === 'scientist'} 
                onCheckedChange={(checked) => setUserMode(checked ? 'scientist' : 'novice')}
              />
              <span className={`text-xs font-medium ${userMode === 'scientist' ? 'text-white exoseer-glow-text' : 'text-white/60'}`}>Scientist Mode</span>
            </div>
            {/* Removed redundant non-functional settings button */}
          </div>
        </div>
      </header>
